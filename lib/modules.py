...
class LRM(nn.Module):
    ...
    def forward(self, input_en: torch.Tensor, input_de: torch.Tensor):
        b, c, h, w = input_de.size()
        input_en = input_en.view(b, self.c_en, -1)
        input_de = input_de.view(b, self.c_de, -1)
        # Channel Resampling
        energy = input_de.matmul(input_en.transpose(-1, -2))
...
class FLM(nn.Module):
    ...
    def forward(self, edge_feature, x, pred):
        ...
        out = self.cbam(out)
        ##############新加################
        feat_local = self.gamma * self.lrm(x, pred) + x
        out = self.conv_fusion(self.alpha * out + self.beta * feat_local)
        return out
